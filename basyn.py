...
def slaveMode():
    ...
    sys.stderr = open('/tmp/error.log', 'w')
        ...
        while True:
            ...
            # INIT command. It gets device file name, hash name, opens the file and returns size
                ...
                (fileNameLength, hashNameLength, bufferSize, chunkSize, compLevel) = struct.unpack('<QQQQB',
                                                                                                   cin.read(8 * 4 + 1))
                # ...and strings
                filename_bytes = cin.read(fileNameLength)
                hashname_bytes = cin.read(hashNameLength)
...
def prepareSlave(settings):
    ...
    file = open('/tmp/remote_scripts.py','w')
...
def initRemoteDevice(io, settings):
    ...
    remotePath_bytes = settings.remotePath.encode('utf-8')
    hashName_bytes = settings.hashName.encode('ascii')
    io.write(struct.pack("<QQQQB", len(remotePath_bytes), len(hashName_bytes), settings.bufferSize,
                         settings.chunkSize, settings.zipLevel))
    io.write(remotePath_bytes)
